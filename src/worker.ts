...
export default function worker(command: string, args: string[], options: EachOptions, callback: EachCallback): void {
  ...
  packageLayers(options, (err, entries, graph) => {
    if (err) return callback(err as EachError);
    if (!entries || entries.length === 0) return callback(undefined, []);
    ...
    const session = entries.length >= 2 && process.stdout.isTTY && typeof createSession === 'function' && !options.streaming && interactive ? createSession({ header, showStatusBar: true, interactive }) : null;
    ...
    if (options.topological) {
      schedule(graph as DependencyGraph<PackageEntry>, (entry, _id, cb) => spawnEntry(entry, cb), { concurrency, failDependents: options.failDependents }, finalize);
    }
